...
course_list = [
  ...
  ['Maths Basics', 'HTML is the foundation of all web pages. Without HTML, you wouldn’t be able to organize text or add images or videos to your web pages.
    HTML is the beginning of everything you need to know to create engaging web pages!',
    'https://images.pexels.com/photos/1619844/pexels-photo-1619844.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260', 3],
...
level_list = [
  ['Basic', 1, 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/1280px-HTML5_logo_and_wordmark.svg.png', Course.first.id],
  ['Elements and Structure', 2, 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/1280px-HTML5_logo_and_wordmark.svg.png', Course.first.id],
...
lecture_list = [
  ...
  ['Create a Table', 'Before displaying data, you must first create the table that will contain the data by using the <table> element.', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQCnOoevg7j9aLRTlea-8ml4w51RTkLrnOT-QZKlMesOk7yY16lag', 2, Level.first.id],
  ['Table Rows', 'In many programs that use tables, the table is already predefined for you, meaning that it contains the rows, columns, and cells that will hold data. In HTML, all of these components must be created.', 'https://higherlogicdownload.s3.amazonaws.com/JAMASOFTWARE/UploadedImages/419b250b-4dd2-48f6-a082-f5391ee4239e/Screen%20Shot%202016-12-06%20at%2012.07.56.png', 3, Level.first.id],
  ['HTML Structure','HTML is organized as a collection of family tree relationships. When an element is contained inside another element,it is considered the child of that element. The child element is said to be nested inside of the parent element.',"https://res.cloudinary.com/dxx1c1rby/image/upload/v1566488892/html_structure_kjenxj.png",1, Level.second.id],
  ['Heading Elements in HTML', 'Headings in HTML are similar to headings in other types of media. For example, in newspapers, large headings are typically used to capture a reader’s attention. Other times, headings are used to describe content, like the title of a movie or an educational article.',  "https://res.cloudinary.com/dxx1c1rby/image/upload/v1566488891/html_headings_skzed1.png", 2, Level.second.id],
  ['The Body', 'One of the key HTML elements we use to build a webpage is the body element. Only content inside the opening and closing body tags can be displayed to the screen.',  'https://res.cloudinary.com/dxx1c1rby/image/upload/v1566488892/html_body_oedwyr.png', 3, Level.second.id],
...
question_list = [
  ...
  ['What is HTML used for?', 'CSS is for the style so HTML is for [...]? :)', 'https://upload.wikimedia.org/wikipedia/commons/6/61/HTML5_logo_and_wordmark.svg', 1, Level.second.id],
  ['How many headings are available in HTML?', '3*2 = ? ;)', 'https://upload.wikimedia.org/wikipedia/commons/6/61/HTML5_logo_and_wordmark.svg', 2, Level.second.id],
  ['Is it possible to display content outside of the body tag?', 'think', 'https://upload.wikimedia.org/wikipedia/commons/6/61/HTML5_logo_and_wordmark.svg', 3, Level.second.id],
...
answer_list = [
  ...
  ['To collect informationS from our user', false, Question.find_by(id: 1).id],
  ['To present tabular data to users', true, Question.find_by(id: 1).id],
  ['For fun', false, Question.find_by(id: 1).id],
  ['<table></table>', true, Question.find_by(id: 2).id],
  ['<body></body>', false, Question.find_by(id: 2).id],
  ['<form></form>', false, Question.find_by(id: 2).id],
  ['<td></td>', false, Question.find_by(id: 3).id],
  ['<th></th>', false, Question.find_by(id: 3).id],
  ['<tr></tr>', true, Question.find_by(id: 3).id],
  ['HTML provides structure to the content appearing on a website (images, text, videos)', true, Question.find_by(id: 4).id],
  ['HTML provides style to the content of our webpage', false, Question.find_by(id: 4).id],
  ['HTML allow the client (my computer + my browser) and a server to communicate', false, Question.find_by(id: 4).id],
  ['2', false, Question.find_by(id: 5).id],
  ['6', true, Question.find_by(id: 5).id],
  ['4', false, Question.find_by(id: 5).id],
  ['It depends', false, Question.find_by(id: 6).id],
  ['Yes', false, Question.find_by(id: 6).id],
  ['No', true, Question.find_by(id: 6).id],
